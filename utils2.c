...
/*   Updated: 2024/10/17 15:29:44 by mailinci         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	end(t_mlx *game)
{
    if (game->map)
        free_map(game->map);
    destroy_image(game);
    mlx_clear_window(game->data, game->window); //clear prima di distruggi altrimenti indirizzo non raggiungibile
    if (game->window)
        mlx_destroy_window(game->data, game->window);
    if (game->data)
        mlx_destroy_display(game->data);
    //mlx_loop_end(game->data); //serve? non serve?
    free(game->data);
    game->data = NULL;
    game->map = NULL;
	exit(0);
